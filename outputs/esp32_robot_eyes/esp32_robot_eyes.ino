...
<body>
  <main>
    ...
    <!-- NEW PHONE TELEMETRY DISPLAY -->
    <div class="telemetry-box">
      <div>Mood: <strong id="liveMood" style="color: #55d67a;">Neutral</strong></div>
      <div>Uptime: <strong id="liveUptime">0s</strong></div>
    </div>
  </main>
  <script>
    ...
    // --- LIVE TELEMETRY POLLING ENGINE ---
    // Automatically loops every 2 seconds on your phone to read AIBI's mind
    setInterval(async () => {
      try {
        const response = await fetch('/state');
        if (response.ok) {
          const data = await response.json();
          document.getElementById('liveMood').textContent = data.mood;
          document.getElementById('liveUptime').textContent = data.uptime_seconds + 's';
        }
      } catch (error) {
        // Fails silently if connection dips temporarily
      }
    }, 2000);
  </script>
</body>
...
void handleState() {
  ...
  // Calculate the current dominant mood
  String mood = "Neutral";
  bool isSleeping = (quietTime > 55000) || (now < serialSleepUntil);
  
  if (isSleeping) {
    mood = "Sleepy";
  } else if (now < happyUntil || now < autoSmileUntil) {
    mood = "Happy";
  } else if (now < talkUntil) {
    mood = "Talking";
  } else if (now < wakeUntil) {
    mood = "Waking";
  } else if (now < attentionUntil) {
    mood = "Curious";
  }

  // Construct a lightweight JSON payload without external libraries
  String json = "{";
  json += "\"uptime_seconds\":" + String(now / 1000) + ",";
  json += "\"mood\":\"" + mood + "\",";
  json += "\"is_sleeping\":" + String(isSleeping ? "true" : "false") + ",";
  json += "\"quiet_time_ms\":" + String(quietTime);
  json += "}";

  // Add CORS headers so React Native can fetch it easily
